...
Page({
    ...
    formSubmit: function(e) {
        ...
        console.log(catogory,date,currentTime,amount,e.detail.value.message)
        if (catogory == undefined || date.length == 0 || currentTime.length == 0 || amount.length == 0 || e.detail.value.message.length == 0) {
            wx.showToast({
                title: '请补全信息！',
                icon: 'none',
                duration: 1500
            ...
            wx.request({
                url: api.AddMeetupXiaoyuan,
                method:'GET',
                data: {
                  category:catogory,
                  due_datetime:date+" "+currentTime,
